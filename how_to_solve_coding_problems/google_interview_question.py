...
def slightly_optimized_pair_sum(array, sum):
    for i in range(len(array)):
        complement = sum - array[i]
        if binary_search(array, i+1, len(array)-1, complement):
            return 'Yes'
    return 'No'

print(slightly_optimized_pair_sum(array, sum))
...
def smartest_pair_sum(array, sum):
    ...
    for item in array:
        complement = sum - item
        if not complement in dictionary:
